...
bool av::Decoder::sendPacket(const AVPacket *packet) {
...
av::FrameUPtr av::Decoder::getFrame() {
    if (!codec_ctx_) throw std::logic_error(errMsg("decoder was not initialized yet"));
...
const AVCodecContext *av::Decoder::getContext() const { return codec_ctx_.get(); }

std::string av::Decoder::getName() const {
